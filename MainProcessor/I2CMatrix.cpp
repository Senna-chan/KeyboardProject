...
void I2CMatrixClass::begin() {
	...
	rowChip.writeRegister(0xFF, MCP23017_IPOLA);

	colChip.writeRegister(0x00, MCP23017_IODIRA);
	colChip.writeRegister(0x00, MCP23017_IODIRB);
	colChip.writeGPIOAB(0x0000);
...
void I2CMatrixClass::scanKeys() {
	...
	for (byte c = 0; c<sizeKpd.columns; c++) {
		pin_mode(c, OUTPUT, true);
		pin_write(c, LOW, true);	// Begin column pulse output.
...
//		}
		...
		pin_write(c, HIGH, true);
		pin_mode(c, INPUT, true);
	}
}
